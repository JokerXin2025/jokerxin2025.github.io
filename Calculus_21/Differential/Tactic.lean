/-
    «Calculus_21».Differential.Tactic
    Released under MIT license as described in the file LICENSE.
    Authors: JokerXin
-/

import «Calculus_21».Expr.Simp
  ...
  eq := by
    ...
    calc
      ...
      _  =  the (0 * x₀ - k * 1) / the (x₀ ^ 2)
            := rfl
      _  =  the ((0 * x₀ - k * 1) / x₀ ^ 2)
            := LimitValue.finite_iff.mp (LimitValue.finite_div h_den_ne0)
      _  =  the (-k / x₀ ^ 2)
            := by ring_nf

private instance deriv_patch₄' {f : ℝ → ℝ} {k x₀ D₁ : ℝ} {c : Prop}
    [h_f : AutoDeriv f x₀ D₁ c]
  : AutoDeriv ((k / ·) ∘ f) x₀ (-k * D₁ / f x₀ ^ 2) (c ∧ f x₀ ≠ 0) where
  eq := sorry
  ...
  eq := by
    ...
    calc
         ...
         =. D f x₀ + D g x₀
            := DerivExpr.Add
      _  =  the D₁ + the D₂
            := by rw [AutoDeriv.eq h₁, AutoDeriv.eq h₂]
      _  =  the (D₁ + D₂)
            := rfl
...
private instance deriv_compAbs {f : ℝ → ℝ} {x₀ D₁ : ℝ} {c : Prop}
  ...
  eq := by
    ...
    exact LimitValue.finite_iff.mp h_chain
...
/-- ## Derivative Calculator
    ...
    - Only used for derivative expression, including
      - `DerivExpr`
      - `LeftDerivExpr` (not yet)
      - `RightDerivExpr` (not yet)
      - `NthDerivExpr` (not yet)
    ...
    - `deriv` requires some side-conditions to exist in the context, which are the
      sum of the corresponding conditions for these different functions:
...
macro "deriv" : tactic => `(tactic| (
  ...
  try focus (change the _ = the _; congr 1 <;> ring)
))
