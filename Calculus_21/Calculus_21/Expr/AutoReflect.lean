...
import «Calculus_21».Expr.Domain
...
/-- Extract the complete unary backward precondition from a proper result. -/
macro "proper_backward" root:term "with" op:term "as" h:ident : tactic => `(tactic|
  have $h := Expr.ProperBackward.unary (op := $op) $root
)

/-- Extract the complete binary backward precondition from a proper result. -/
macro "proper_backward₂" root:term "with" op:term "as" h:ident : tactic => `(tactic|
  have $h := Expr.ProperBackward.binary (op := $op) $root
)
