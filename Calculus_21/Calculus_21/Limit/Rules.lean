...
section
variable {A B C : RSequence} {F G H : RFunction} {n : ℕ} {k x₀ u₀ L₁ L₂ : ℝ}

/-- Scalar Multiplication of RSequence Limit -/
...
/-- Scalar Multiplication of RFunction Limit -/
...
/-- Additive Inverse of RSequence Limit -/
...
/-- Additive Inverse of RFunction Limit -/
...
/-- Multiplicative Scalar Power of RSequence Limit -/
...
/-- Multiplicative Scalar Power of RFunction Limit -/
...
/-- Multiplicative Inverse of RSequence Limit -/
...
/-- Multiplicative Inverse of RFunction Limit -/
...
/-- Subtraction of RSequence Limit -/
...
/-- Multiplication of RSequence Limit -/
...
/-- Multiplication of RFunction Limit -/
...
/-- Composition of RFunction Limit -/
...
/-- Squeeze Theorem for RSequence Limit -/
...
/-- Squeeze Theorem for RFunction Limit -/
...
private theorem function_add_comm (F G : RFunction) : F + G = G + F := by
  apply RFunction.ext
  · funext _
    exact add_comm _ _
  · ext _
...
private theorem sequence_add_comm (A B : RSequence) : A + B = B + A := by
  apply RSequence.ext
  · funext _
    exact add_comm _ _
  · exact max_comm _ _
...
variable {A B : RSequence} {F G : RFunction} {x₀ L : ℝ}
...
/- RFunction addition rules at a finite point. -/
...
private theorem unionDomainToInfty {K : RFunction}
...
private theorem function_sub_eq_add_neg (F G : RFunction) : F - G = F + (-G) := by
  apply RFunction.ext
  · funext x; change F.map x - G.map x = F.map x + -G.map x; ring
  · rfl

private theorem sequence_sub_eq_add_neg (A B : RSequence) : A - B = A + (-B) := by
  apply RSequence.ext
...
private theorem function_mul_comm (F G : RFunction) : F * G = G * F := by
  apply RFunction.ext
  · funext x; exact mul_comm _ _
  · ext x; exact and_comm

private theorem sequence_mul_comm (A B : RSequence) : A * B = B * A := by
  apply RSequence.ext
...
/- RSequence multiplication rules. -/
