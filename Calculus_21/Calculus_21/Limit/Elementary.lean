...
/-- Constant RFunction's Continuity -/
protected lemma Continuity.Constant {C : ℝ}
  : (Constant C).isContinuous
:= by
  intro _ h_x₀
...
/-- Identity RFunction's Continuity -/
protected lemma Continuity.Identity
  : Identity.isContinuous
:= by
  intro x₀ h_x₀
  unfold isContinuousAt FuncLimit
  refine ⟨h_x₀, ?_⟩
...
/-- Absolute Value RFunction's Continuity -/
...
/-- Square Root RFunction's Continuity for `x > 0` -/
protected lemma Continuity.Sqrt
  : ∀ x > 0, Sqrt.isContinuousAt x
:= by
  intro x₀ h_x₀
  prove_continuity
...
/-- Square Root RFunction's Right Continuity at `0` -/
...
/-- Power RFunction's Right Continuity at `0` -/
...
/-- Power RFunction's Continuity for `n : ℤ` -/
protected lemma Continuity.Power_ℤ {n : ℤ}
  : (NPower n).isContinuous
:= by
  intro x₀ h_x₀
  cases n with
  | ofNat m =>
    ...
    | succ m =>
      have hid : Identity.isContinuousAt x₀ := Continuity.Identity x₀ (mem_univ _)
      have hp : (Identity ^ (m + 1)).isContinuousAt x₀ :=
  ...
  | negSucc m =>
    ...
    have hinv : (Identity ^ (m + 1))⁻¹.isContinuousAt x₀ := infer
...
/-- Natural Exponential RFunction's Continuity -/
...
/-- Exponential RFunction's Continuity -/
protected lemma Continuity.Expow {a : ℝ}
    (h_a : a > 0)
  : (Expow a).isContinuous
:= by
  intro x₀ _
  rw [← Expow_eq h_a]
  infer
...
/-- Logarithm RFunction's Continuity -/
...
/-- Power RFunction's Continuity for `x > 0` -/
protected lemma Continuity.Power {a : ℝ}
  : ∀ x > 0, (Power a).isContinuousAt x
:= by
  intro x₀ h_x₀
  have hcomp : (Exp ⊙ (Ln * Constant a)).isContinuousAt x₀ :=
    infer
            Ln.isContinuousAt x₀
...
/-- Tangent RFunction's Continuity -/
...
/-- Cotangent RFunction's Continuity -/
...
/-- Cosecant RFunction's Continuity -/
...
/-- Hyp-Sine RFunction's Continuity -/
...
/-- Hyp-Cotangent RFunction's Continuity -/
...
/-- Hyp-Secant RFunction's Continuity -/
...
/-- Hyp-Cosecant RFunction's Continuity -/
...
/-- Arc-Sine RFunction's Continuity for `x > -1 ∧ x < 1` -/
protected lemma Continuity.Arcsin
  : ∀ x ∈ Ioo (-1) 1, Arcsin.isContinuousAt x
:= by
  intro x₀ h_x₀
  let α := arcsin x₀
...
/-- Arc-Sine RFunction's Right Continuity at `-1` -/
...
/-- Arc-Sine RFunction's Left Continuity at `1` -/
...
/-- Arc-Cosine RFunction's Continuity for `x > -1 ∧ x < 1` -/
protected lemma Continuity.Arccos
  : ∀ x ∈ Ioo (-1) 1, Arccos.isContinuousAt x
:= by
  intro x₀ h_x₀
  have hsub : (Constant (π / 2) - Arcsin).isContinuousAt x₀ := by
    apply Sub
...
/-- Arc-Cosine RFunction's Right Continuity at `-1` -/
protected lemma RightContinuity.Arccos_neg1
  : Arccos.isRightContinuousAt (-1)
:= by
...
/-- Arc-Cosine RFunction's Left Continuity at `1` -/
protected lemma LeftContinuity.Arccos_1
  : Arccos.isLeftContinuousAt 1
:= by
  refine ⟨by norm_num [Arccos], ?_⟩
...
/-- Arc-Tangent RFunction's Continuity -/
...
/-- Arc-Cotangent RFunction's Continuity -/
protected lemma Continuity.Arccot
  : Arccot.isContinuous
:= by
  intro x₀ _
  have hc : (Constant (π / 2)).isContinuousAt x₀ :=
    Continuity.Constant _ (mem_univ _)
  have hsub : (Constant (π / 2) - Arctan).isContinuousAt x₀ :=
    Sub hc (Continuity.Arctan x₀ (mem_univ _))
  rw [← Arccot_eq]
  exact hsub

/-- Arc-Secant RFunction's Continuity for `x < -1 ∨ x > 1` -/
protected lemma Continuity.Arcsec
  : ∀ x ∈ Iio (-1) ∪ Ioi 1, Arcsec.isContinuousAt x
:= by
  intro x₀ h_x₀
  rcases h_x₀ with h_x₀ | h_x₀
  · change x₀ < -1 at h_x₀
    have h_x₀_ne : x₀ ≠ 0 := by linarith
    have hcomp : (Arccos ⊙ Identity⁻¹).isContinuousAt x₀ :=
      infer
              Identity.isContinuousAt x₀
           => Identity⁻¹.isContinuousAt x₀
...
/-- Arc-Secant RFunction's Left Continuity at `-1` -/
protected lemma LeftContinuity.Arcsec_neg1
  : Arcsec.isLeftContinuousAt (-1)
:= by
...
/-- Arc-Secant RFunction's Right Continuity at `1` -/
protected lemma RightContinuity.Arcsec_1
  : Arcsec.isRightContinuousAt 1
:= by
  refine ⟨by norm_num [Arcsec], ?_⟩
...
/-- Arc-Cosecant RFunction's Continuity for `x < -1 ∨ x > 1` -/
protected lemma Continuity.Arccsc
  : ∀ x ∈ Iio (-1) ∪ Ioi 1, Arccsc.isContinuousAt x
:= by
  intro x₀ h_x₀
  rcases h_x₀ with h_x₀ | h_x₀
  · change x₀ < -1 at h_x₀
    have h_x₀_ne : x₀ ≠ 0 := by linarith
    have hcomp : (Arcsin ⊙ Identity⁻¹).isContinuousAt x₀ :=
      infer
              Identity.isContinuousAt x₀
           => Identity⁻¹.isContinuousAt x₀
              := ⟨⟨(Continuity.Identity x₀ (mem_univ _)).1, h_x₀_ne⟩,
...
/-- Arc-Cosecant RFunction's Left Continuity at `-1` -/
protected lemma LeftContinuity.Arccsc_neg1
  : Arccsc.isLeftContinuousAt (-1)
:= by
...
/-- Arc-Cosecant RFunction's Right Continuity at `1` -/
protected lemma RightContinuity.Arccsc_1
  : Arccsc.isRightContinuousAt 1
:= by
  refine ⟨by norm_num [Arccsc], ?_⟩
  have h_arcsin_left := LeftContinuity.Arcsin_1.2
  constructor
