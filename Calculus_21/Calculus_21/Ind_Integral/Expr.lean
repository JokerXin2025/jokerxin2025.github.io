...
lemma HasAntideriv.differ_by_const {f F G : RFunction}
...
/-! # Bridges: `HasAntideriv` (RFunction) ↔ `HasAntiderivRaw` (ℝ → ℝ) -/

theorem HasAntiderivRaw.to_Function {f F : ℝ → ℝ}
    (h : HasAntiderivRaw f F)
  : HasAntideriv ⟨f, Iii⟩ ⟨F, Iii⟩ := h

theorem HasAntideriv.to_Raw {f F : RFunction}
  ...
  have hF_eq : ⟨F.map, Iii⟩ = F := by
    apply RFunction.ext <;> simp [h_dom]
  rw [hF_eq] at hDeriv
  exact h x h_x d hDeriv

theorem HasAntideriv.mono {f F G : RFunction}
