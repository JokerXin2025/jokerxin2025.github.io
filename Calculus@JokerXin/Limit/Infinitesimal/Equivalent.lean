...
lemma ExpEquiv {f : ℝ → ℝ}
    (h_ifs : _lim f 0 = the 0)
  : _lim ((fun x ↦ e ^ f x - 1) / f) 0 = the 1
:= sorry
...
theorem EquivSubst {f f' g : ℝ → ℝ} {x₀ : ℝ}
    (h_equiv : _lim (f / f') x₀ = the 1)
  : _lim (fun x ↦ f x / g x) 0 =? _lim (fun x ↦ f' x / g x) 0
:= sorry
