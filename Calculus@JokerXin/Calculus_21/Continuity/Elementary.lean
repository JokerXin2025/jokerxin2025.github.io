/-
    Calculus_21.Continuity.Elementary
...
:= by
  ...
  · exact h_x₀
  · constructor
    · exact ⟨1, zero_lt_one, subset_univ _⟩
    · intro ε _
      use 1
      constructor
      · exact zero_lt_one
...
:= by
  ...
  exact (Constant_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Identity_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Abs_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Sqrt_isContinuous x₀ h_dom).2
...
:= by
  ...
  · rw [h_0]
    ...
    exact Sqrt_isRightContinuous_0.2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Exp_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Expow_isContinuous h_dom x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Ln_isContinuous x₀ h_dom).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Log_isContinuous h_dom.2 x₀ h_dom.1).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Sin_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Cos_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Tan_isContinuous x₀ h_dom).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Cot_isContinuous x₀ h_dom).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Sec_isContinuous x₀ h_dom).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Csc_isContinuous x₀ h_dom).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Sinh_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Cosh_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Tanh_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Coth_isContinuous x₀ h_dom).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Sech_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Csch_isContinuous x₀ h_dom).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Arcsin_isContinuous x₀ h_dom).2
...
:= by
  ...
  · rw [h_1]
    ...
    exact Arcsin_isLeftContinuous_1.2
...
:= by
  ...
  · rw [h_neg1]
    apply RightLimit_to_RightLimitExpr
    exact Arcsin_isRightContinuous_neg1.2
  · have h_dom' : x₀ > -1 ∧ x₀ < 1 := by
      constructor
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Arccos_isContinuous x₀ h_dom).2
...
:= by
  ...
  · rw [h_1]
    apply LeftLimit_to_LeftLimitExpr
    exact Arccos_isLeftContinuous_1.2
  · have h_dom' : x₀ > -1 ∧ x₀ < 1 := by
      constructor
      · exact h_dom.left
      · exact lt_of_le_of_ne h_dom.right h_1
...
:= by
  ...
  · rw [h_neg1]
    apply RightLimit_to_RightLimitExpr
    exact Arccos_isRightContinuous_neg1.2
  · have h_dom' : x₀ > -1 ∧ x₀ < 1 := by
      constructor
      · exact lt_of_le_of_ne h_dom.left (Ne.symm h_neg1)
      · exact h_dom.right
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Arctan_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Arccot_isContinuous x₀ (mem_univ x₀)).2
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Arcsec_isContinuous x₀ h_dom).2
...
:= by
  ...
  · rw [h_neg1]
    apply LeftLimit_to_LeftLimitExpr
    exact Arcsec_isLeftContinuous_neg1.2
  · have h_dom' : x₀ < -1 ∨ x₀ > 1 := by
...
:= by
  ...
  · rw [h_1]
    apply RightLimit_to_RightLimitExpr
    exact Arcsec_isRightContinuous_1.2
  · have h_dom' : x₀ < -1 ∨ x₀ > 1 := by
      rcases h_dom with h_le | h_gt
      · left
...
:= by
  apply FuncLimit_to_FuncLimitExpr
  exact (Arccsc_isContinuous x₀ h_dom).2
...
:= by
  ...
  · rw [h_neg1]
    apply LeftLimit_to_LeftLimitExpr
    exact Arccsc_isLeftContinuous_neg1.2
  · have h_dom' : x₀ < -1 ∨ x₀ > 1 := by
      rcases h_dom with h_le | h_gt
      · left
        exact Std.lt_of_le_of_ne h_le h_neg1
...
:= by
  ...
  · rw [h_1]
    apply RightLimit_to_RightLimitExpr
    exact Arccsc_isRightContinuous_1.2
  · have h_dom' : x₀ < -1 ∨ x₀ > 1 := by
      rcases h_dom with h_le | h_gt
      · left
        exact RCLike.ofReal_lt_ofReal.mp h_le
